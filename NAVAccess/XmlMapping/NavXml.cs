...
namespace LSOmni.DataAccess.BOConnection.NavSQL.XmlMapping.Loyalty
{
    ...
    public class NavXml : BaseXml
    {
        ...
        public ContactRs ContactCreateResponseXML(string responseXml)
        {
            /*
            ...
            XmlNode node = document.SelectSingleNode("//ContactID");
            if (node == null)
                throw new XmlException("ContactID node not found in response xml");
            contactRs.ContactId = node.InnerText;

            node = document.SelectSingleNode("//AccountID");
            if (node == null)
                throw new XmlException("AccountID node not found in response xml");
            contactRs.AccountId = node.InnerText;

            node = document.SelectSingleNode("//SchemeID");
            if (node == null)
                throw new XmlException("SchemeID node not found in response xml");
            contactRs.SchemeId = node.InnerText;

            node = document.SelectSingleNode("//CardID");
            if (node == null)
                throw new XmlException("CardID node not found in response xml");
            contactRs.CardId = node.InnerText;

            node = document.SelectSingleNode("//ClubID");
            if (node == null)
                throw new XmlException("ClubID node not found in response xml");
            contactRs.ClubId = node.InnerText;

            if (true)
            // if (NavLoyalty.NAVVersion.Major > 7)
            {
                node = document.SelectSingleNode("//Total_Remaining_Points");
                if (node == null)
                    throw new XmlException("Total_Remaining_Points node not found in response xml");
                decimal balance = 0L;
                if (string.IsNullOrWhiteSpace(node.InnerText) == false)
                    balance = ConvertTo.SafeDecimal(node.InnerText);
                contactRs.Balance = balance;
            }
            else
            {
                contactRs.Balance = 0.0M; //for NAV70, OK to return 0 pts when user is created.
            }
            return contactRs;
        }
        ...
        public ContactRs LoginResponseXML(string responseXml)
        {
            #region xml
            /*
			<?xml version="1.0" encoding="utf-8" standalone="no" ?>
			- <Response>
				...
				- <Response_Body>
					<LoginID>tom</LoginID>
					<Password>hJJLNLi0LGSoV//Jy4wi/WG4Vz36hwlhbeSjpsBS76g99VaVCGssjUXPIzPftm24sCWJnAOES0Fm0RfVADN5ow==</Password>
					<AccountID />
					<ContactID />
					<Email />
					<FirstName />
					...
					<CardID />
					- <Login_Card_Buffer>
						<ClubCode>CRONUS</ClubCode>
						<ClubDescription>Cronus Loyalty Club</ClubDescription>
						<CardNo>10078</CardNo>
						<CardholderName>Tom Thomson</CardholderName>
					...
					- <Login_Card_Buffer>
						...
						<ClubDescription>Cronus Loyalty Club</ClubDescription>
						<CardNo>10079</CardNo>
						<CardholderName>Sabina Garcia</CardholderName>
						<Blocked>false</Blocked>
					</Login_Card_Buffer>
				</Response_Body>
			 */
            ...
            XElement cEl = doc.Element("Response").Element("Response_Body").Element("AccountID");
            if (cEl != null)
            {
                contactRs.AccountId = cEl.Value;
            }
            cEl = doc.Element("Response").Element("Response_Body").Element("ContactID");
            if (cEl != null)
            {
                contactRs.ContactId = cEl.Value;
            }
            cEl = doc.Element("Response").Element("Response_Body").Element("Email");
            if (cEl != null)
            {
                contactRs.EMail = cEl.Value;
            }
            cEl = doc.Element("Response").Element("Response_Body").Element("FirstName");
            if (cEl != null)
            {
                contactRs.FirstName = cEl.Value;
            }
            cEl = doc.Element("Response").Element("Response_Body").Element("LastName");
            if (cEl != null)
            {
                contactRs.LastName = cEl.Value;
            }

            //if I find the cardId then no need to look at card buffer
            cEl = doc.Element("Response").Element("Response_Body").Element("CardID");
            if (cEl != null)
            {
                contactRs.CardId = cEl.Value;
                if (string.IsNullOrWhiteSpace(contactRs.CardId) == false)
                    return contactRs;
            }

            IEnumerable<XElement> cardsEl = doc.Element("Response").Element("Response_Body").Descendants("Login_Card_Buffer");
            foreach (XElement cardEl in cardsEl)
            {
                if (cardEl.Element("CardNo") == null)
                    throw new XmlException("CardNo node not found in response xml");
                contactRs.CardId = cardEl.Element("CardNo").Value;
                if (string.IsNullOrWhiteSpace(contactRs.CardId) == false)
                    break; //simply take the first non blank one
            }
        ...
        public string TestConnectionResponseXML(string responseXml)
        {
            /*
            <Response>
              <Request_ID>TEST_CONNECTION</Request_ID>
